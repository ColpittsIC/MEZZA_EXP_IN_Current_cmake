...
hal_spi_handle_t *mx_spi2_init(void)
{
  ...
  /* This board is the SPI Slave: the remote board (Master) drives SCK and NSS. */
  ...
  spi_config.baud_rate_prescaler = HAL_SPI_BAUD_RATE_PRESCALER_64; /* ignored in Slave mode: clock comes from the Master */
  spi_config.first_bit           = HAL_SPI_MSB_FIRST;
  spi_config.nss_pin_management  = HAL_SPI_NSS_PIN_MGMT_INPUT; /* NSS pin is the hardware chip-select input */
